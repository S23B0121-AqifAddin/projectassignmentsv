...
# --- SECTION 2: RESPONSIBILITY & MATURITY ---
with st.container(border=True):
    st.header("🛡️ Behavioral Analysis")
    c3, c4 = st.columns(2)
    with c3:
...
# --- SECTION 3: CORRELATION & INCOME ---
with st.container(border=True):
    st.header("📈 Economic Decision Factors")
    c5, c6 = st.columns([2, 1]) # Heatmap gets more space
    with c5:
        st.write("**Correlation of Financial Behaviours**")
        
        num_cols = ['Organised_Money_Management', 'Saver_or_Spender', 'Buy_on_Credit',
...
with st.expander("View Project Context"):
    ...
    st.write("**Problem:** Dissatisfaction does not always lead to action.")
